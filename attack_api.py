...
generator_save_path = ''            # put your adversary model path here
...
def query_classifier(text):
    ...
    # if you want to use proxy to attack
    proxies = {
    ...
    try:
        ...
        if 'documents' in response.json() and len(response.json()['documents']) > 0:
            predicted_class = response.json()['documents'][0].get('predicted_class', 'Unknown')
            if predicted_class == "ai":
                print("Label == 1, attack failed!")
                return 1
            elif predicted_class == "human":
                print("Label == 0, attack success!")
                return 0
            else:
                print("Label == 2, attack success!")
                return 2
        else:
            print("Fail to fetch document data")
...
def get_important_token_indices(model, input_ids, num_important_tokens):
    ...
    token_grad_norms = torch.norm(grad, dim=-1)  
...
def generate_adversarial_text_perturbation(input_text, max_iterations=60, num_important_tokens_start=2, epsilon_start=0.8, epsilon_max=1.0, epsilon_step=0.05):
    ...
    for step in range(max_iterations):
        ...
        if adversarial_class != original_class:
            ...
            print("Attack Fail.")
        # epsilon = min(epsilon + epsilon_step, epsilon_max)        # Optional
        num_important_tokens = min(num_important_tokens + 5, len(input_ids[0]))
    else:
        print("Attack Fail.")
...
def generate_adversarial_text_mask(input_text, max_iterations=50, num_important_words=2):
    ...
    return marked_adversarial_text, unpruned_adversarial_text  
...
def process_dataset(dataset, args, output_file_template):
    ...
    output_path = os.path.join("dataset/attack", output_file)
                ...
                while retries < max_retries:
                    try:
                        if args.method == 'perturbation':
                            adversarial_text, unpruned_adversarial_text = generate_adversarial_text_perturbation(text, max_iterations=20)
                        elif args.method == 'mask':
                            adversarial_text, unpruned_adversarial_text = generate_adversarial_text_mask(text, max_iterations=50)
                        ...
                        attack_success = (query_classifier(adversarial_text) != 1)
                        total_queries += query
                        ...
                        print(f"Error, retrying: {retries + 1} , error: {e}")
                        raise e
            else:
                print("SKIPPED.")
                continue
